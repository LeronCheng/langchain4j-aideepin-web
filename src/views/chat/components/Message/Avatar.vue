...
<template>
  ...
  <NAvatar v-else-if="name && name.includes('deepseek')" :size="imageSize ? imageSize : 32" object-fit="contain" round :src="deepseekAvatar" color="#fff" />
  <!-- <span v-else-if="name === 'ollama'">
    <svg t="1714878417759" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="11596" width="32" height="32">
      <path d="M512 0a512 512 0 1 0 512 512A512 512 0 0 0 512 0zM272.896 688.128a102.4 102.4 0 0 1-112.64-88.576V424.96a102.4 102.4 0 0 1 112.64-89.088H430.08l-31.232 40.448-112.64 27.648c-17.92 4.096-34.304 12.288-34.304 27.648v159.232c0 14.848 15.872 23.552 34.304 27.648l107.52 22.016 36.864 47.616z m107.52-132.096V467.968h263.168v88.064z m483.84 43.52a102.4 102.4 0 0 1-113.152 88.576h-158.72l36.864-47.616 107.52-22.016c18.432-4.096 34.304-12.8 34.304-27.648V431.616c0-15.36-16.384-23.552-34.304-27.648l-112.64-27.648-31.232-40.448h157.184a102.4 102.4 0 0 1 114.176 89.088z" fill="#4D4D4D" p-id="11597" />
    </svg>
  </span>
  <span v-else-if="name === 'dashscope'">
    <svg t="1714878323762" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="10469" width="32" height="32">
      <path d="M442.368 525.312V583.68h-47.104s-47.104 4.096-63.488 57.344c-5.12 35.84 5.12 57.344 7.168 61.44 2.048 4.096 16.384 30.72 55.296 37.888h88.064V526.336l-39.936-1.024z m-1.024 179.2h-35.84s-25.6-1.024-32.768-29.696c-3.072-12.288 1.024-26.624 3.072-32.768 2.048-6.144 10.24-19.456 27.648-25.6h37.888v88.064zM604.16 587.776v113.664h-40.96s-14.336-2.048-16.384-13.312v-101.376l-41.984 1.024v112.64s2.048 28.672 41.984 38.912h101.376V587.776H604.16z" fill="#4D4D4D" p-id="10470" />
      <path d="M512 48.128C258.048 48.128 53.248 253.952 53.248 507.904s204.8 458.752 458.752 458.752 458.752-205.824 458.752-458.752c0-252.928-204.8-459.776-458.752-459.776z m23.552 223.232c4.096-37.888 49.152-95.232 84.992-87.04s67.584 55.296 61.44 96.256c-6.144 40.96-38.912 94.208-89.088 88.064-50.176-6.144-61.44-52.224-57.344-97.28zM430.08 167.936c37.888 0 68.608 44.032 68.608 97.28 0 54.272-30.72 97.28-68.608 97.28-36.864 0-67.584-44.032-68.608-97.28 0-54.272 30.72-97.28 68.608-97.28zM217.088 398.336s8.192-79.872 63.488-84.992c44.032-4.096 76.8 44.032 79.872 71.68 2.048 18.432 11.264 100.352-57.344 114.688-67.584 14.336-93.184-64.512-86.016-101.376z m521.216 345.088c-36.864 88.064-174.08 41.984-174.08 41.984s-50.176-16.384-108.544-3.072c-59.392 13.312-109.568 8.192-109.568 8.192s-68.608 2.048-88.064-84.992c-19.456-87.04 68.608-134.144 74.752-142.336s51.2-38.912 80.896-88.064c29.696-49.152 117.76-88.064 179.2 8.192C638.976 548.864 716.8 609.28 716.8 609.28s59.392 45.056 21.504 134.144z m-12.288-201.728c-74.752 2.048-77.824-50.176-77.824-88.064 0-39.936 8.192-95.232 68.608-95.232s76.8 59.392 76.8 78.848c0 19.456 8.192 103.424-67.584 104.448z" fill="#4D4D4D" p-id="10471" />
    </svg>
  </span> -->
  <NAvatar v-else-if="name && name.includes('qwen')" :size="imageSize ? imageSize : 32" object-fit="contain" round :src="qianWen" color="#fff" />
  <NAvatar v-else-if="name && name.includes('claude')" :size="imageSize ? imageSize : 32" object-fit="contain" round :src="claude" color="#fff" />
  <NAvatar v-else-if="name && name.includes('grok')" :size="imageSize ? imageSize : 32" object-fit="contain" round :src="grok" color="#fff" />
  <NAvatar v-else-if="name && name.includes('doubao')" :size="imageSize ? imageSize : 32" object-fit="contain" round :src="doubao" />
  <NAvatar v-else-if="name && name.includes('gemini')" :size="imageSize ? imageSize : 32" object-fit="contain" round :src="ollamaAvatar" />
  ...
  <span v-else class="text-[28px] dark:text-white">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true" :width="imageSize ? imageSize : '1em'" :height="imageSize ? imageSize : '1em'">
